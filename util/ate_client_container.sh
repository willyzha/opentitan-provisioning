...
# ate_client_stage_bundle <context_dir> <harness_exe> <ate_dll> [runtime_dll...]
#
# Lays out the Windows bundle that gets installed into the container image. The
# harness, ate.dll and the MinGW runtime DLLs must all sit in the same
# directory, mirroring the layout produced by `//src/ate:windows`.
ate_client_stage_bundle() {
  ...
  cp -f "${harness_exe}" "${context_dir}/bundle/ate_dll_smoke.exe"
  cp -f "${ate_dll}" "${context_dir}/bundle/ate.dll"
  local dll
  for dll in "$@"; do
    cp -f "${dll}" "${context_dir}/bundle/"
  done
...
ate_client_report_status() {
  ...
  if [[ "${status}" -eq 42 ]]; then
    ...
    echo "  Known 32-bit MinGW toolchain hazard; see docs/windows_ate_dll.md."
  else
    echo "FAIL[${name}]: ate_dll_smoke.exe exited with status ${status}."
  fi
  return 1
}
