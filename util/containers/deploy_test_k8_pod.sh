...
if [ -f "${TOKEN_INIT_SCRIPT}" ]; then
    ...
    if [[ "${DEPLOY_ENV}" == "dev" ]]; then
        ...
        ${TOKEN_INIT_SCRIPT} --action "spm-sku-init" --sku sival --sku sival_mldsa --sku cr01 --sku pi01 --sku ti01
    fi
    echo " -- Step 5: Generate the CSRs for all SKUs."
    ${TOKEN_INIT_SCRIPT} --action "spm-sku-csr" --sku sival --sku sival_mldsa --sku cr01 --sku pi01 --sku ti01
    ...
    ${TOKEN_INIT_SCRIPT} --action "offline-sku-certgen" --sku sival --sku sival_mldsa --sku cr01 --sku pi01 --sku ti01

    echo "Print object attributes ..."
    ${TOKEN_INIT_SCRIPT} --action "spm-init" --show
    ${TOKEN_INIT_SCRIPT} --action "offline-common-init" --show
    ${TOKEN_INIT_SCRIPT} --action "spm-sku-init" --sku sival --sku sival_mldsa --sku cr01 --sku pi01 --sku ti01 --show
fi
